...
import 'package:daggerheart_game_master_companion/ui/shared/alert_dialogs.dart';
...
class AddCharacterDialog extends StatefulWidget {
  const AddCharacterDialog({this.playerCharacter, super.key});
...
class _AddCharacterDialogState
    extends DaggerheartState<AddCharacterDialog, AddCharacterDialogViewModel> {
  late TextEditingController nameTextController;

  @override
  void initState() {
    viewModel.initialize(widget.playerCharacter);
    ...
    viewModel.eventStream.listen(
      ...
      onError: (error) {
        ...
        showErrorDialog(context, localizations.addCharacterDialogSavingError);
      },
    );
    super.initState();
  }
          ...
          child: Column(
            ...
            children: [
              Column(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(
                      localizations.addCharacterDialogCharacterName,
                  ...
                  TextField(
                    decoration: InputDecoration(
                      ...
                      hintText: uiState.isNameError
                          ? localizations.addCharacterDialogHintError
                          : null,
                      border: OutlineInputBorder(),
                      // bordo outline
                      enabledBorder: OutlineInputBorder(
                        borderSide: BorderSide(
                          color: uiState.isNameError ? Colors.red : Colors.grey,
                        ),
                      ),
                      focusedBorder: OutlineInputBorder(
                        borderSide: BorderSide(
                          color: uiState.isNameError ? Colors.red : Colors.blue,
                          width: 2,
                        ),
                      ),
              ...
              Row(
                ...
                children: [
                  _createDropdown(
                    uiState.classUiModel,
                    450,
                    localizations.daggerheartClass,
                    localizations.addCharacterDialogSelectClass,
                    (selectedChild) {
                      viewModel.classChanged(selectedChild);
                    },
                  ),
                  _createDropdown(
                    uiState.subclassUiModel,
                    450,
                    localizations.subclass,
                    localizations.addCharacterDialogSelectSubclass,
                    (selectedChild) {
              ...
              Row(
                ...
                children: [
                  _createDropdown(
                    uiState.ancestryUiModel,
                    450,
                    localizations.ancestry,
                    localizations.addCharacterDialogSelectAncestry,
                    (selectedChild) {
                      viewModel.ancestryChanged(selectedChild);
                    },
                  ),
                  _createDropdown(
                    uiState.communityUiModel,
                    450,
                    localizations.community,
                    localizations.addCharacterDialogSelectCommunity,
                    (selectedChild) {
              ...
              Row(
                spacing: 20.0,
                children: [
                  _createDropdown(uiState.agilityUiModel, 130, localizations.agility, "", (
                    selectedChild,
                  ) {
                    viewModel.traitChanged(ChoiceType.Agility, selectedChild);
                  }),
                  _createDropdown(uiState.strengthUiModel, 130, localizations.strength, "", (
                    selectedChild,
                  ) {
                    viewModel.traitChanged(ChoiceType.Strength, selectedChild);
                  }),
                  _createDropdown(uiState.finesseUiModel, 130, localizations.finesse, "", (
                    selectedChild,
                  ) {
                    viewModel.traitChanged(ChoiceType.Finesse, selectedChild);
                  }),
                  _createDropdown(uiState.instinctUiModel, 130, localizations.instinct, "", (
                    selectedChild,
                  ) {
                    viewModel.traitChanged(ChoiceType.Instinct, selectedChild);
                  }),
                  _createDropdown(uiState.presenceUiModel, 130, localizations.presence, "", (
                    selectedChild,
                  ) {
                    viewModel.traitChanged(ChoiceType.Presence, selectedChild);
                  }),
                  _createDropdown(uiState.knowledgeUiModel, 130, localizations.knowledge, "", (
                    selectedChild,
                  ) {
  ...
  Widget _createDropdown(
    ChoiceUiModel uiModel,
    double width,
    String title,
    String hint,
    Function(ChoiceChildUiModel?) onSelectedValueChanged,
  ) {
    final theme = Theme.of(context);
    final localizations = AppLocalizations.of(context)!;
    return Column(
  ...
  List<Widget> _createDomainAbilitiesDropdowns(LoadedAddCharacterDialogUiState uiState) {
    ...
    final firstDomainDropdown = _createDropdown(
      uiState.firstDomainAbilityUiModel,
      width,
      localizations.domainAbility,
      localizations.addCharacterDialogSelectDomain,
      (selectedChild) {
        viewModel.domainAbilityChanged(ChoiceType.FirstDomainAbility, selectedChild);
      },
    );
    final secondDomainDropdown = _createDropdown(
      uiState.secondDomainAbilityUiModel,
      width,
      localizations.domainAbility,
      localizations.addCharacterDialogSelectDomain,
      (selectedChild) {
